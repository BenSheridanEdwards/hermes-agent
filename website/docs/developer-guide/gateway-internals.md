...
- That line is emitted by the `cron.scheduler` logger, so `_ComponentFilter` in `hermes_logging.py` keeps it out of `gateway.log`: it lands in `logs/agent.log`. A failed run or a real delivery failure logs at `WARNING`, so it also reaches `logs/errors.log` and the gateway's stderr (`gateway.error.log` under launchd, the journal under systemd — the stderr handler is added only under `-v`/`-q` and sits at `WARNING` at verbosity 0, `gateway/run.py`). A successful run on the origin-less `origin` lane is not a failure and logs at `INFO`, keeping the 2 MB error log for actual errors; that line reaches stderr only under `-v`, and disappears from the files entirely when `logging.level` is set to `WARNING`. Log files run the `RedactingFormatter`, so the body is redacted unless `security.redact_secrets` is off, and lone surrogates from `surrogateescape`-decoded job output are replaced before the record is built (the file handlers have no `errors=` and would otherwise drop the record).
