...
delegation:
  ...
  # fallback_policy: "pinned"                # "pinned" (default) fails loudly on a pinned provider; "inherit_then_parent" opts into inherited fallbacks plus the parent's primary as terminal recovery.
...
**Fallback policy:** A pinned `delegation.provider` fails loudly by default rather than silently moving a quiet-mode child onto the parent's routes. Set `delegation.fallback_policy: inherit_then_parent` only when that rerouting is intentional. Hermes then removes any duplicate of the delegated primary, preserves the parent's configured fallback order, and appends the parent's effective primary runtime as a distinct terminal recovery route. Credentials remain internal to the runtime route and are never emitted through configuration readback.
