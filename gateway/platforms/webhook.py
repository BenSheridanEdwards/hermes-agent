...
class WebhookAdapter(BasePlatformAdapter):
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        """Prefer cross-platform voice when this webhook session delivers elsewhere.

        Webhook has no native audio transport. The base fallback turns failed
        voice into a Telegram-visible "Couldn't deliver the audio attachment"
        via deliver:telegram. Route voice to the deliver target instead.
        """
        ...
        if deliver_type in {"", "log", "github_comment"}:
            # No human chat target — drop quietly (do not poison deliver path).
            return SendResult(success=True)
        ...
        try:
            ...
            return SendResult(success=False, error=f"Unknown deliver platform: {deliver_type}: {exc}")
        ...
        if not target_chat:
            return SendResult(success=False, error=f"No chat_id for webhook voice deliver:{deliver_type}")
