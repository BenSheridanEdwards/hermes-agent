...
def write_runtime_status(
    *,
    ...
    elif current_record["start_time"] is None:
        # A reused PID with no start-time fingerprint is indistinguishable from
        # the previous process. Fail closed by discarding volatile receipts.
        for existing_platform in payload.get("platforms", {}).values():
...
def _sanitize_platform_runtime_receipt(value: Any) -> Optional[dict[str, Any]]:
    ...
    if (
        ...
        or len(bot_id) > 128
        or (bot_id and (not bot_id.isascii() or not bot_id.isdigit()))
    ):
        raise ValueError("platform runtime receipt bot_id must be a short string")
    bot_username = value["bot_username"]
    if bot_username is not None and (
        not isinstance(bot_username, str)
        or len(bot_username) > 128
        or not bot_username.isascii()
        or any(not (character.isalnum() or character == "_") for character in bot_username)
    ):
        raise ValueError(
